...
bool chifoumi::transfererScrore()
{
    if(db->openDataBase())
    if(creatTable())
    if(remplirTable())
...
bool chifoumi::creatTable()
{
    QSqlQuery query;

    if(!query.exec("create table TableauChifoumi (Horodatage varchar(19) primary key, nomJoueurHumain varchar(30), scoreJoueurHumain int(3),joueurMachine varchar(7), scoreJoueurMachine int(3), gagnant varchar(7));")){
        qDebug()<<"table déjà créer";
        return true;
    }
    else {
        query.exec("create table TableauChifoumi (Horodatage varchar(19) primary key, nomJoueurHumain varchar(30), scoreJoueurHumain int(3),joueurMachine varchar(7), scoreJoueurMachine int(3), gagnant varchar(7));");
        qDebug()<<"table créer";
        return true;
    }
}

bool chifoumi::remplirTable()
{
    QString victoire;
    if(gagnant==0) victoire="Machine";else if(gagnant==1) victoire="Joueur";else victoire="Null";
    QSqlQuery query;
    QString insertions="insert into TableauChifoumi "
    " values(:Horodatage, :nomJoueurHumain, :scoreJoueurHumain, :joueurMachine, :scoreJoueurMachine, :gagnant);";
    query.prepare(insertions);
    query.bindValue(":Horodatage", QDateTime::currentDateTime().toString("dd/MM/yyyy hh:mm:ss"));
    query.bindValue(":nomJoueurHumain", nom);
    query.bindValue(":scoreJoueurHumain", ui->scoreJoueur->text().toInt());
    query.bindValue(":joueurMachine", "Machine");
    query.bindValue(":scoreJoueurMachine", ui->scoreMachine->text().toInt());
    query.bindValue(":gagnant", victoire);
    if(!query.exec())
        return false;
    else
        return true;
